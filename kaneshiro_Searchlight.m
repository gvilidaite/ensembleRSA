...
outputFile = 'SearchlightKaneshiro';
...
NTimeWindows = 9; % number of time windows to separate the data into if
...
if doSearchlight == 1
    ...
    for n = 1:length(files)
        ...
        for elec = 1:size(currentfile.X_3D,1) % loop over the electrode dimension
            if (timepoints == 1) % Whole trial, electrode x trial data (timepoints averaged)
                currentX = squeeze(currentfile.X_3D(elec,:,:))';
            ...
            elseif (timepoints == 2)
                [windowedData indexes] = gv_make_sliding_windows(currentfile.X_3D, NTimeWindows, 1);
...
if (timepoints == 1)
    save(strcat(savepath, outputFile),'CM','accuracy','predY','pVal','classifierInfo','RSAinfo');
else
    save(strcat(savepath, outputFile),'CM','accuracy','predY','pVal','classifierInfo','RSAinfo','windowsforplotting');
end
