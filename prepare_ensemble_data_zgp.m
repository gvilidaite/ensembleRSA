...
EEGpath = '/Volumes/Denali_4D2/2015_Ensembles/Ensembles_MATLAB/';
outputpath = 'prepared_ensemble_data/'; % point this to where you want the prepared files saved - usually a local directory
...
for s = 1:length(subjects)
    ...
    allSeg = dir(strcat(EEGpath,subjects{s},'/RTSeg*.mat')); % get info (names) of all segment files
    ...
    for n = 1:length(allSeg)
        ...
        load(strcat(EEGpath,subjects{s},'/',listSegs{n}));
    ...
    for c = 1:6 % for each condition
        ...
        allRaw = dir(strcat(EEGpath,subjects{s},'/Raw_c00',num2str(c),'_t*')); % get info (names) of all segment files
        for t = 1:length(allRaw)
        
            count = count + 1;

            EEG = load(strcat(EEGpath,subjects{s},'/','Raw_c',num2str(c,'%03d'),'_t',num2str(t,'%03d'),'.mat')); % loading the file
